...
const { Sessions, UserRoles } = require('../models');
...
const authenticateAdminToken = async (req, res, next) => {
    ...
    try {
        ...
        // Verify if the user has an Admin (id_role = 2) or Superadmin (id_role = 1) role
        const userRole = await UserRoles.findOne({
            where: { id_user: decodedToken.id_user, id_role: [1, 2] }, // Check if the user's role is either 1 or 2
        });
